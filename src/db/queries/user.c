...
int user_get_management_ui_setups(db_handle_t *db, long long user_account_pin,
    ...
    const char *sql =
        "SELECT DISTINCT "
        "  D.code_name, D.display_name, "
        "  C.id, C.code_name, C.display_name, "
        "  F.address "
        "FROM " TBL_USER_ACCOUNT " A "
        "LEFT JOIN " TBL_CLIENT_USER " B ON A.pin = B.user_account_pin "
        "JOIN " TBL_CLIENT " C ON B.client_pin = C.pin "
        "  OR C.is_universal = " BOOL_TRUE " "
        "JOIN " TBL_ORGANIZATION " D ON C.organization_pin = D.pin "
        "JOIN " TBL_CLIENT_RESOURCE_SERVER " E ON C.pin = E.client_pin "
        "JOIN " TBL_RESOURCE_SERVER " F ON E.resource_server_pin = F.pin "
        "JOIN " TBL_CLIENT_REDIRECT_URI " G ON C.pin = G.client_pin "
        "WHERE A.pin = " P"1 "
        ...
        "  AND F.address = " P"3 "
        "ORDER BY D.code_name ASC, C.code_name ASC "
